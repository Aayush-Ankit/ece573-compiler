...
%union {
  ...
  // new types for constructing ast
  ASTNode * ASTNode_t;
}
...
%type <str_val> id str;           /* string class */
...
%type <ASTNode_t> expr expr_prefix factor addop factor_prefix postfix_expr mulop primary
assign_expr assign_stmt compop cond init_stmt incr_stmt expr_list expr_list_tail call_expr control_stmt return_stmt;
...
program:  PROGRAM id _BEGIN pgm_body END {  assert (ss.empty() != 1);
                                            curr_scope = ss.top();
                                            curr_scope->update_scopeIds(count); // update scopeIds of un-named blocks (if/else/while/for)
                                            string duplicate = curr_scope->check_duplicate();
                                            if (duplicate != "") {
                                                cout << "DECLARATION ERROR " << duplicate;
                                                cout << "\n\n";
                                            }
                                            else {
                                                // top level program is identified by on ASTNode (ExprList)
                                                //curr_scope->print_stmts();    // -- print AST
                                                updateVarTypeInAST (((curr_scope->getStmts())->front()), global_scope); // update types in VarRef ASTNodes by global symbol-table lookup
                                                PostTraverseAST (((curr_scope->getStmts())->front())); // -- generate IR code for program
                                                ...
                                                cout << ";JSR FUNC_main\n";
                                                cout << ";HALT\n";
                                                print_inst_ASTNode ((curr_scope->getStmts())->front());   // -- print generated IR
                                                //cout << endl;

                                                cout << ";Assembly Code.....\n";
                                                list<string> * ir_code = ((curr_scope->getStmts())->front())->getCodeObjectCode();
                                                print_decl (curr_scope); // -- print declarations
                                                cout << "push" << endl;
                                                cout << "push" << endl;
                                                cout << "push" << endl;
                                                cout << "push" << endl;
                                                cout << "push" << endl;
                                                cout << "jsr FUNC_main" << endl;
                                                cout << "sys halt" << endl;
                                                ToAssembly(*ir_code); // -- print Assembly
                                            }
                                            ss.pop();
                                         };
...
/* String Declarations */
string_decl: STRING id ASSIGN str SEMICOLON { /* create a str_entry object */
                                              $$ = new str_entry($2, $4);
                                            };
...
/* Complex statements and Condiiton */
